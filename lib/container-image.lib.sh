...
function setContainerCmd() {
    ...
    [ -z "$containerCmd" ] && err container command could not be found && return 1
...
function login2aws() {
    ...
    debug "Login to AWS..."
    # login to AWS
    set -o pipefail # makes the pipeline return the status of the last (rightmost) command to exit non-zero, or zero if all succeed
