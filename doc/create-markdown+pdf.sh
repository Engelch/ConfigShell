...
[ "$minor" -lt 1 ] && echo 1>&2 'ERROR: md2pdf at least required in version 0.1.0, exiting' && exit 3

find . -maxdepth 1 -name '*.orig.md' -print0 | while read -r -d '' origFile; do
...
find . -maxdepth 1 -name '*.marp.md' -print0 | while read -r -d '' origFile; do
