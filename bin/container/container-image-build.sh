...
#########################################################################################
# Load the default settings for all scripts
...
#########################################################################################
# Load the settings for all container-related scripts
...
function createBuildPackages() {
    ...
    # Copy everything else from the parent directory if it is not...
    for file in ../* ; do
        [ -d "$file" ] \
        && [ "$file" != ../Container ] \
        && [ "$file" !=  ../build ] \
...
function main() {
    ...
    if [ "$(uname)" = Darwin ] ; then
        exitIfBinariesNotFound gtar
        debug tar set to gtar
        _tar=gtar
    else
        debug tar stays OS tar
    fi
