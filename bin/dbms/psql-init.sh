...
function helpExit() {
    ...
    err '  ' $(basename $0) '-n init name=psql66  # create a container and a volume named psql66'
    err '  ' $(basename $0) '-n name=psql66 init  # same as command before'
    err '  ' $(basename $0) '-n -l psql66 init    # same as command before'
    exit 2
}

function initVarsLibs() {
  #########################################################################################
...
# parseArgs parses the command line arguments and returns the remaining command line in the variable args
function parseArgs() {
  if [ "${1:-}" = '-n' ]; then
    err DRY run mode
    DRY=echo
  ...
  while true ; do
    ...
    l|list|list)
      shift
      debug set CMD to list
      [ -n "$CMD" ] && errorExit 7 cmd to be set to list, but already set to $CMD
      CMD=list
      ;;
    stat|status)
...
function main() {
  initVarsLibs
  parseArgs "$@" # returns remaining command line in args, no $* or $@ after this call
  ...
  case "$CMD" in 
    delete)
      container.sh stop $args "$POSTGRESQL_CONTAINER_LABEL"
      container.sh container rm "$POSTGRESQL_CONTAINER_LABEL"
      container.sh volume rm "$CONTAINER_VOLUME"
      ;;
    init)
      debug startContainer "$POSTGRESQL_CONTAINER_LABEL" "$CONTAINER_VOLUME" $args
      startContainer "$POSTGRESQL_CONTAINER_LABEL" "$CONTAINER_VOLUME" $args
      ;;
    list)
      container.sh ps -a | grep postgres
      ;;
    start) 
      container.sh start $args"$POSTGRESQL_CONTAINER_LABEL"
      ;;
    status)
      local -r output=$(container.sh  ps -q  --filter name="${POSTGRESQL_CONTAINER_LABEL}" $args)
      [ -z "$output" ] && exit 1
