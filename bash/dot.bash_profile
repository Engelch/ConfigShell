...
function setupPath1() {
    ...
    debug8 set up initial path
    PATH=/sbin:/usr/sbin:/bin:/usr/bin:/usr/local/sbin:/usr/local/bin
    if [ "$UID" = 0 ] ; then
        debug8 PATH overwrite initialisation for user root
...
function envVars() {
    ...
    export RSYNC_SLINK_FLAGS="$RSYNC_FLAGS --copy-links" # copy s-links as files
...
function main() {
    ...
    # setup for system-wide installations
    [ -d /usr/local/cargo ] && export CARGO_HOME=/usr/local/cargo
    [ -d /usr/local/rustup ] && export RUSTUP_HOME=/usr/local/rustup
    # else
